/**
 * ReaxFF Parameter File Reader
 * Reads and parses ReaxFF force field parameter files
 */

/**
 * Atom type parameters constructor
 */
export class AtomType {
  constructor(paramAtom) {
    this.atomID = paramAtom[0];
    this.roSigma = parseFloat(paramAtom[1]) || 0;    // ro(sigma) single bond covalent radius
    this.valency = parseFloat(paramAtom[2]) || 0;    // valency of atom
    this.atmcMass = parseFloat(paramAtom[3]) || 0;   // atomic mass in daltons
    this.rvdw = parseFloat(paramAtom[4]) || 0;       // van der Waals radius
    this.dij = parseFloat(paramAtom[5]) || 0;        // epsilon
    this.gamma = parseFloat(paramAtom[6]) || 0;      // gammaEEM
    this.roPi = parseFloat(paramAtom[7]) || 0;       // covalent radius 2
    this.valE = parseFloat(paramAtom[8]) || 0;       // val(e)
    this.alpha = parseFloat(paramAtom[9]) || 0;
    this.gammaW = parseFloat(paramAtom[10]) || 0;    // gamma(w)
...
/**
 * Global parameters constructor
 */
export class ParamGlobal {
...
export class BondType {
  constructor(paramBond) {
    ...
    // Will be populated from one-body parameters
    this.roSigma = 0;
    this.roPi = 0;
    this.roPiPi = 0;
    this.pboc3 = 0;
...
/**
 * Clean array by removing falsy values and create appropriate type
 * @param {Array} actual - Array to clean
 * @param {string} type - Type of parameter to create
 * @returns {Object} Constructed parameter object
 */
function cleanArray(actual, type) {
  ...
  switch (type) {
    case 'atom':
      return new AtomType(newArray);
    case 'bond':
      return new BondType(newArray);
    case 'angle':
      return new Angles(newArray);
    case 'torsions':
      return new Torsions(newArray);
    case 'diag':
      return new Diagonal(newArray);
    case 'hbonds':
      return new HBonds(newArray);
    default:
      return null;
  }
}
 ...
 * @param {...number} dimensions - Array dimensions
...
function combineParameters(onebody_parameters, twobody_parameters) {
  for (let i = 0; i < onebody_parameters.length; i++) {
    for (let j = 0; j < onebody_parameters.length; j++) {
      ...
      paramAtom1.lgcij = Math.sqrt((p1.lgcij || 0) * (p2.lgcij || 0));
      paramAtom1.lgre = 2.0 * Math.sqrt((p1.lgre || 0) * (p2.lgre || 0));
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = () => {
        try {
          ...
          this.parameters = parseParameterFile(text);
          resolve({
            r_ij: this.r_ij,
            ...this.parameters
          });
...
export default ReaxFFFileReader;
